...
class DEVOLUCION(Frame):
    ...
    def inicio(self):
        self.contenedor = Frame(self, bg="#FAEBD7", width= 1100)
        self.contenedor.pack_propagate(False)
        self.contenedor.pack(expand=True, fill="y", pady= 20)

        #TEXTO DEL INICIO
        self.cont_texto=Frame(self.contenedor, bg="#FAEBD7", height=180, width=1200)
        self.cont_texto.pack(expand=True)

        Label(self.cont_texto, text="¡Gracias por venir!", font=("Arial", 30), bg="#FAEBD7").pack()

        #donde esta el desplegable de proyectos y el long text para poner la devolucion
        self.cont_devolucion=Frame(self.contenedor, bg="#FAEBD7", width=1200)
        self.cont_devolucion.pack(expand=True, fill = "x", padx= 180)
        self.datos_comentario = {}

        cont_desplegable = Frame(self.cont_devolucion, bg="#FAEBD7")
        ...
        Label(cont_desplegable, text="Proyecto: ", font=("Arial", 13), bg="#FAEBD7",).grid(row=0, column=0, padx=10, sticky="w")
        ...
        Label(self.cont_devolucion, text="Devolución: ", font=("Arial", 13), bg="#FAEBD7").pack(pady=5, padx=10, anchor="w")
        self.texto = Text(self.cont_devolucion, height=10, width=80, wrap='word', font=("Robot", 12))
        self.texto.pack(padx=10, anchor="w") 

        self.contador_label = Label(self.cont_devolucion, text="0/750 caracteres", font=("Arial", 10), bg="#FAEBD7")
        ...
        self.cont_interesados=Frame(self.contenedor, bg="#FAEBD7", width=1200)
        self.cont_interesados.pack(expand=True)

        contenedor_pregunta = Frame(self.cont_interesados, bg="#FAEBD7")
        contenedor_pregunta.pack(expand=True, fill="x", ipady=20)
        Label(contenedor_pregunta, text="¿Interesado en ser estudiante de ISAUI?", font=("Arial", 14), bg="#FAEBD7").grid(row=0, column=0, padx=10, sticky="w")
        self.respuesta = StringVar()
        self.respuesta.set(1)
        self.respuesta.trace("w", self.actualizar_estado_campos)
        Radiobutton(contenedor_pregunta, text="Sí", variable=self.respuesta, value=1, bg="#FAEBD7", font=("Arial", 13)).grid(row=0, column=1, sticky="w")
        Radiobutton(contenedor_pregunta, text="No", variable=self.respuesta, value=2, bg="#FAEBD7", font=("Arial", 13)).grid(row=0, column=2, sticky="w")

        #campos para poner los datos del interesado
        cont_campos = Frame(self.cont_interesados, bg="#FAEBD7", height=100, width=1200)
        ...
        for i, campo in enumerate(campos_arriba):
            Label(cont_campos, text=campo, font=("Arial", 13), bg="#FAEBD7").grid(row=1, column=i, padx=10, pady=5, sticky="w")
        ...
        for j, campo in enumerate(campos_abajo):
            Label(cont_campos, text=campo, font=("Arial", 13), bg="#FAEBD7").grid(row=3, column=j, padx=10, pady=5, sticky="w")
            if campo == "Carrera":
                self.entradas[campo] = ttk.Combobox(cont_campos, state="readonly", width=27, font=("Arial", 12))
        ...
        contenedor_botones = Frame(self.contenedor, bg="#FAEBD7")
        contenedor_botones.pack(pady=20)

        btn_guardar = Button(contenedor_botones, text="Guardar", font=("Arial", 15), bg="MistyRose3", width=15, command=self.guardar)
        btn_guardar.grid(row=0, column=1, padx= 20)

        btn_cancelar = Button(contenedor_botones, text="Cancelar", font=("Arial", 15), bg="MistyRose3", width=15, command=self.cancelar)
